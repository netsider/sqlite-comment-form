...
if (isset($_POST['submit'])){
	...
	if (strlen($_POST['TEXT']) >= 1){
		$db->exec("CREATE TABLE $TABLE (Id INTEGER PRIMARY KEY, Title VARCHAR, TextData TEXT, Date VARCHAR, SID VARCHAR)");    
		$db->exec("INSERT INTO $TABLE (Title, TextData, Date, SID) VALUES ('$TEXT', '$TITLE', '$DATE', '$sid');");
		//$db->exec("INSERT INTO $TABLE (Title, TextData, Date) VALUES ('$TEXT', '$TITLE', '$DATE');");
		//$db->exec("INSERT INTO $TABLE (SessID) VALUES ('$sid');");
	}
}else{
	$TEXT = "";
	$TITLE = "";
