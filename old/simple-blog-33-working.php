...
if (isset($_POST['submit'])){
	...
	if (strlen($_POST['TEXT']) >= 1){
		...
		//check for duplicate entries
		 //$del = $db->query("SELECT Time FROM $TABLE WHERE SessID = $sid ORDER BY Id DESC LIMIT 1");
		  //echo $del->rowCount();
		 $result = $db->query("SELECT Time FROM $TABLE WHERE SessID = $sid ORDER BY Id DESC LIMIT 1");
